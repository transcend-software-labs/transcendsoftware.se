	...
	_, err := p.pool.Exec(ctx,
		`INSERT INTO projects
		   (id, user_id, name, brief, status, questions, design_options, design_brief,
		    ...
		    domain_intent, domain_intent_buy, domain_cost_ore, preview_host, domain_paid_through, planner_profile, impl_profile, domain_prepaid)
		...
		pr.DomainIntent, pr.DomainIntentBuy, pr.DomainCostOre, pr.PreviewHost, nullableTime(pr.DomainPaidThrough), pr.PlannerProfile, pr.ImplProfile, pr.DomainPrepaid)
	return err
}
...
func (p *Postgres) UpdateProject(ctx context.Context, pr *project.Project) error {
	tag, err := p.pool.Exec(ctx,
		`UPDATE projects SET
		   ...
		   domain_intent=$44, domain_intent_buy=$45, domain_cost_ore=$46, preview_host=$47, domain_paid_through=$48, planner_profile=$49, impl_profile=$50, domain_prepaid=$51
		 WHERE id=$1`,
...
func scanProject(row rowScanner) (*project.Project, error) {
	...
	var paidAt, domainCreatedAt, domainVerifiedAt, changePeriodStart, deliveredAt, domainPaidThrough *time.Time // NULL when unset
	err := row.Scan(&pr.ID, &pr.UserID, &pr.Name, &pr.Brief, &pr.Status, &questionsJSON,
		...
		&pr.DomainIntent, &pr.DomainIntentBuy, &pr.DomainCostOre, &pr.PreviewHost, &domainPaidThrough, &pr.PlannerProfile, &pr.ImplProfile, &pr.DomainPrepaid)
	if err != nil {
		return nil, err
	}
