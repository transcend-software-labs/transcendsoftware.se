	...
	_, err := p.pool.Exec(ctx,
		`INSERT INTO projects
		   (id, user_id, name, brief, status, questions, design_options, design_brief,
		    ...
		    screenshots, findings, critique, iterations_used, created_at, updated_at, plan_spec, locale, content_answers, content_rosters, pending_images, image_gen_count, paid, paid_at, paid_via, content_pending)
		...
		pr.Paid, nullableTime(pr.PaidAt), pr.PaidVia, pr.ContentPending)
	return err
}
...
func (p *Postgres) UpdateProject(ctx context.Context, pr *project.Project) error {
	tag, err := p.pool.Exec(ctx,
		`UPDATE projects SET
		   ...
		   repo_url=$13, snapshot_key=$14, screenshots=$15, findings=$16, critique=$17, iterations_used=$18, updated_at=$19, plan_spec=$20, locale=$21, content_answers=$22, content_rosters=$23, pending_images=$24, image_gen_count=$25, paid=$26, paid_at=$27, paid_via=$28, content_pending=$29
		 WHERE id=$1`,
...
func scanProject(row rowScanner) (*project.Project, error) {
	...
	err := row.Scan(&pr.ID, &pr.UserID, &pr.Name, &pr.Brief, &pr.Status, &questionsJSON,
		...
		&pr.Answers, &pr.Plan, &pr.Verdict, &pr.RejectReason, &pr.PreviewURL, &pr.RepoURL,
		&pr.SnapshotKey, &screenshotsJSON, &findingsJSON, &pr.Critique, &pr.IterationsUsed, &pr.CreatedAt, &pr.UpdatedAt, &specJSON, &pr.Locale, &contentJSON, &rostersJSON, &pendingJSON, &pr.ImageGenCount, &pr.Paid, &paidAt, &pr.PaidVia, &pr.ContentPending)
	if err != nil {
		return nil, err
	}
	if paidAt != nil {
